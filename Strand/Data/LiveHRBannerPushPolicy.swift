...
enum LiveHRBannerPushPolicy {
    ...
    static func due<Content: Equatable>(shown: Content?, next: Content, sinceLastPush: TimeInterval,
                                        staleAfter: TimeInterval, background: Bool = false) -> Bool {
        guard sinceLastPush > minimumSpacing else { return false }
        // Preserve the fork's slower background cadence while sharing upstream's content gate.
        guard !background || BackgroundWorkPolicy.activityDue(
            elapsed: sinceLastPush, changed: true, background: true) else { return false }
