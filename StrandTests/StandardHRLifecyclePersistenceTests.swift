...
final class StandardHRLifecyclePersistenceTests: XCTestCase {
    ...
    func testQuietModeKeepsSummaryErrorsAndRecoveryWithoutPerReadingLogs() async {
        let store = CountingStore()
        var clock: TimeInterval = 0
        var lines: [String] = []
        let collector = Collector(store: store, deviceId: "test", log: { lines.append($0) },
                                  now: { 1_750_000_000 }, monotonic: { clock })
        collector.detailedDiagnostics = { false }
        for tick in 0...60 {
            clock = Double(tick)
            collector.ingestStandardHR(hr: 70, rr: [], at: 1_750_000_000 + tick)
            await collector.flushStandardHR()
        }
        XCTAssertFalse(lines.contains { $0.contains("host-received") || $0.contains("flush-attempt") || $0.contains("flush-succeeded") })
        XCTAssertEqual(lines.filter { $0.contains("standard-hr summary") }.count, 1)
        store.failNext = true
        collector.ingestStandardHR(hr: 71, rr: [], at: 1_750_000_061)
        await collector.flushStandardHR()
        XCTAssertTrue(lines.contains { $0.contains("Live persist FAILED") })
        await collector.flushStandardHR()
        XCTAssertTrue(lines.contains { $0.contains("persistence recovered") })
    ...
    func testBackgroundLifecycleFlushesSubThresholdStandardHRAndLogsStoreCounts() async {
        ...
        collector.detailedDiagnostics = { true }
...
// The release Collector below is a test-only oracle, extracted verbatim from v11.8.0
// except for its type name. Compare actual stored streams, including failed-insert retries.
@MainActor
final class ConservativeCollectorParityTests: XCTestCase {
    ...
    func testStoredStreamsMatchStockAcrossDiagnosticsFamiliesAndFailures() async {
        for detailed in [false, true] {
            for family: DeviceFamily in [.whoop4, .whoop5] {
                let actual = CaptureStore(), expected = CaptureStore()
                var clock: TimeInterval = 0
                let patched = Collector(store: actual, deviceId: "test", now: { 1_750_000_000 }, monotonic: { clock })
                patched.detailedDiagnostics = { detailed }
                let stock = StockCollectorOracle(store: expected, deviceId: "test", now: { 1_750_000_000 }, monotonic: { clock })
                for tick in 0..<240 {
                    clock = Double(tick)
                    let hr = [0, 29, 30, 72, 130, 220, 221][tick % 7]
                    let rr = [249, 250, 800 + tick, 3000, 3001]
                    let contact: StandardHRContact = tick % 3 == 0 ? .supportedNotDetected : .supportedDetected
                    let ts = 1_750_000_000 + tick
                    patched.ingestStandardHR(hr: hr, rr: rr, contact: contact, family: family, at: ts)
                    stock.ingestStandardHR(hr: hr, rr: rr, contact: contact, family: family, at: ts)
                    actual.failNext = tick % 11 == 0
                    expected.failNext = actual.failNext
                    await patched.flushStandardHR(reason: .background)
                    await stock.flushStandardHR(reason: .background)
                    await patched.flushStandardHR()
                    await stock.flushStandardHR()
                }
                XCTAssertEqual(actual.inserted, expected.inserted)
                XCTAssertEqual(actual.inserted.count, 240)
            }
        }
    }
}
...
@MainActor
private final class StockCollectorOracle {
    ...
    /// test fakes that construct a StockCollectorOracle are untouched; `BLEManager` wires its own `log`.
    ...
    /// rebuilds the StockCollectorOracle (a store rebuild after unlock, among other paths), so a log can carry an
    ...
    /// throws. Mirrors latestHRSampleTs() — the BLE offload hook reads gravity through the StockCollectorOracle
    /// because the StockCollectorOracle owns the concrete store.
    ...
    func ingest(frame: [UInt8], parsed: ParsedFrame) {
        ...
        assert(parsed == parseFrame(frame, family: family),
               "StockCollectorOracle.ingest: threaded ParsedFrame != fresh parse (#47 parse-once invariant)")
        #endif
...
final class ConservativePowerSavingPolicyTests: XCTestCase {
